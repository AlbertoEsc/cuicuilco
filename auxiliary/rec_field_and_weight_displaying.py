...
def compute_receptive_field(flow, output_signal, delta = 0.01):
    zero_input = numpy.zeros((1, flow[0].input_dim))
    unary_input = numpy.identity(flow[0].input_dim)
    
    zero_output = flow.execute(zero_input)[:, output_signal]
    print "zero_output.shape=", zero_output.shape
    unary_output = flow.execute(unary_input*delta)[:, output_signal]
    print "unary_output.shape=", unary_output.shape
    activation = (unary_output-zero_output)/delta
...
def redisplay_rec_field(flow, image_width, image_height, selected_true_layer, selected_node, selected_weight, normalization_type, f2a11):
    print "selected_true_layer = " + str(selected_true_layer),
    ...
    flow_subset = flow[0:selected_true_layer+1]
    if selected_node > 0:
        output_signal = selected_node * flow_subset[selected_true_layer].nodes[0].output_dim + selected_weight 
    else:
        output_signal = selected_weight
    print "output_signal  = " + str(output_signal)
    
    rf = compute_receptive_field(flow_subset, output_signal)
    rfdisp = rf.reshape((image_height, image_width))
    print "rfdisp=", rfdisp

    if normalization_type == 0:
        rfdisp = rfdisp + 0.0
    elif normalization_type == 1:
        rfdisp = numpy.sqrt(numpy.abs(rfdisp))
    else:
        rfdisp = signed_sqrt(rfdisp)

    scale_disp = 1.0
    rfdisp = scale_to(rfdisp, rfdisp.mean(), rfdisp.max()-rfdisp.min(), 127.5, 255.0, scale_disp, 'tanh')
    pic = f2a11.imshow(rfdisp, aspect='auto', interpolation='nearest', origin='upper', cmap=mpl.pyplot.cm.gray)
#   f2a11.set_xlabel(label)
...
if True:
    ...
    #Create Figure
    f1 = plt.figure()
    plt.suptitle("Weight Display System")
    ...
    f2 = plt.figure()
    plt.suptitle("Receptive Fields Display System")
    f2a11 = plt.subplot(1,2,1)
#    f2a11.set_position([0.05, 0.5, 0.04, 0.4])
    plt.title("Receptive Field (Row of matrix A; Ax+b=y)")
    ...
    def on_press(event):
        ...
        redisplay_rec_field(flow, image_width, image_height, true_layers[selected_layer], selected_node, selected_weight, normalization_type, f2a11)
        f1.canvas.draw()
        f2.canvas.draw()
