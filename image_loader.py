...
def create_image_filenames(im_seq_base_dir, slow_signal=0, ids=[0], expressions=[0], morphs=[0], poses=[0],
                           lightings=[0], step=1, offset=0, verbose=False):
    """Creates a list of image filenames randomA_eB_cC_pD_iE.tif, where A, B, C, D, and E are integers.
     
    The values of A, B, C, D, and E spanning all combinations of identities, expressions, morphs, poses and lightings.
    ...
    filenames = []
    for (identity, ex, morph, pose, lighting) in selection:
        filename = im_seq_base_dir + "/random%03d_e%d_c%d_p%03d_i%d.tif" % (identity, ex, morph, pose, lighting)
...
def create_image_filenames2(im_seq_base_dir, slow_signal=0, ids=[0], ages=[999], genders=[999], racetweens=[999],
    ...
    filenames = []
    for (identity, age, gender, racetween, ex, morph, pose, lighting) in selection:
        #        file = im_seq_base_dir + "/random%03d_e%d_c%d_p%03d_i%d.tif"%(id, ex, morph, pose, lighting)
        filename = im_seq_base_dir + "/output%03d_a%03d_g%03d_rt%03d_e%d_c%d_p%d_i%d.tif" % (identity, age, gender,
                                                                                             racetween, ex, morph,
                                                                                             pose, lighting)
        filenames.append(filename)
...
def create_image_filenames3(im_seq_base_dir, im_base_name, slow_signal=0, ids=[0], ages=[999], genders=[999],
    ...
    filenames = []
    for (identity, age, gender, racetween, ex, morph, pose, lighting) in selection:
        filename = im_seq_base_dir + "/" + im_base_name
        if identity is not None:
            if len_ids == 3:
                filename += "%03d" % identity
            elif len_ids == 4:
                filename += "%04d" % identity
            elif len_ids == 5:
                filename += "%05d" % identity
            else:
                filename += "%d" % id
        if age is not None:
            filename += "_a%03d" % age
        if gender is not None:
            filename += "_g%03d" % gender
        if racetween is not None:
            filename += "_rt%03d" % racetween
        if ex is not None:
            filename += "_e%d" % ex
        ...
        if lighting is not None:
            if im_base_name == 'car':
                filename += "_%04d" % lighting
            else:
                filename += "_i%d" % lighting
...
def simple_normalization_from_coords(im, relevant_left, relevant_right, relevant_top, relevant_bottom,
                                     show_points=False, obj_avg=0.0, obj_std=0.2):
...
def simple_normalization_GenderMultiply(im):
    ...
    # print "Originally: im_ar.min(), im_ar.max() , im_ar.max(), im_ar.mean()=", im_ar.min(), im_ar.max(),
    # im_ar.shape, im_ar.mean()
    if RGB:
        goal_mean = 255 * 0.08
    else:
        goal_mean = 0.08
...
def load_image_data_monoprocessor(image_files, image_array, image_width, image_height, subimage_width, subimage_height,
    ...
    #    print subimage_first_column[act_im_num] + translations_x[act_im_num] +
    # subimage_width * pixelsampling_x[act_im_num]
    #    print subimage_first_row[act_im_num] + translations_y[act_im_num] +
    # subimage_height * pixelsampling_y[act_im_num]
        ...
        for image_file in unique_image_files:
            ...
            for act_im_num in act_im_num_indices[image_file]:
                ...
                if delta_ang is not None:
                    ...
                    rotation_window_width = 2 * max(im.size[0] - 1 - rotation_center_x_int + 0.5,
                                                    rotation_center_x_int + 0.5)  # Result is always integer and odd
                    rotation_window_height = 2 * max(im.size[1] - 1 - rotation_center_y_int + 0.5,
                                                     rotation_center_y_int + 0.5)  # Result is always integer and odd

                    if integer_rotation_center is False:
                    ...
                    # Avoiding type problem, even though variables are actually integer
                    ...
                    # (rotation_crop_y1 - rotation_crop_y0):
                    ...
                    if contrast_enhance in ["AgeContrastEnhancement_Avg_Std", "AgeContrastEnhancement",
                                            "GTSRBContrastEnhancement", "ContrastGenderMultiply",
                    ...
                    # NOTE: subimage_coordinates goes from 0 to width!!! typical of python notation. Thus, the
                    # rightmost pixel is ignored, similarly downmost.
...
def load_image_data(image_files, image_array, image_width, image_height, subimage_width, subimage_height,
    ...
    if background_type is not None and color_background_filter is not None:
        ex = "parallel loading unsupported for such background type or background filter. Set number of processors " + \
             "CUICUILCO_IMAGE_LOADING_NUM_PROC to 1 or undefined"
        raise Exception(ex)
    else:
        subimages = numpy.zeros((num_images, subimage_width * subimage_height * pixel_dimensions))
        ...
        for proc_num in range(num_proc):
            ...
            process_args = (unique_image_files_subsets[proc_num], act_im_num_indices, subimage_width,
                            subimage_height, pixel_dimensions, add_noise, convert_format, background_type,
    ...
    if background_type is None or color_background_filter is None:
        ...
        if delta_ang is not None:
            ...
            if integer_rotation_center:
                ...
                rotation_window_width = 2 * max(im.size[0] - 1 - rotation_center_x_int + 0.5,
                                                rotation_center_x_int + 0.5)  # Result is real
                rotation_window_height = 2 * max(im.size[1] - 1 - rotation_center_y_int + 0.5,
                                                 rotation_center_y_int + 0.5)  # Result is real
                rotation_window_width = int(rotation_window_width + 0.5)
                rotation_window_height = int(rotation_window_height + 0.5)
            ...
            # Here crop size should not loose any pixel from the rotation window
            crop_size = (int(rotation_window_width + 0.5), int(
                rotation_window_height + 0.5))  # Avoiding type problem, even though variables are actually integer
            crop_coordinates = (rotation_crop_x0, rotation_crop_y0, rotation_crop_x1, rotation_crop_y1)
            # print crop_size
            # print crop_coordinates

            im_crop_first = im.transform(crop_size, Image.EXTENT, crop_coordinates,
...
def contrast_enhance_image(im_rotated, x0, x1, y0, y1, obj_avg, obj_std, contrast_enhance):
    if contrast_enhance in ["AgeContrastEnhancement_Avg_Std", "AgeContrastEnhancement", "GTSRBContrastEnhancement",
        ...
        if contrast_enhance == "AgeContrastEnhancement_Avg_Std":
            ...
            im_contrasted = simple_normalization_GenderMultiply(simple_normalization_GenderMultiply(
                simple_normalization_GenderMultiply(im_rotated)))  # Normalized 3 times!
        else:
...
def extract_subimage_rotate(image, subimage_coordinates, delta_ang, out_size=(64, 64), allow_out_of_image_sampling=True,
    ...
    if delta_ang is not None:
        ...
        rotation_window_width = 2 * max(image.size[0] - 1 - rotation_center_x_int + 0.5,
                                        rotation_center_x_int + 0.5)  # Result is always integer and odd
        rotation_window_height = 2 * max(image.size[1] - 1 - rotation_center_y_int + 0.5,
                                         rotation_center_y_int + 0.5)  # Result is always integer and odd

        ########
        rxA = (x0 - rotation_center_x) * numpy.cos(delta_ang * numpy.pi / 180.0) + \
              (y0 - rotation_center_y) * numpy.sin(delta_ang * numpy.pi / 180.0)
        ryA = (y0 - rotation_center_y) * numpy.cos(delta_ang * numpy.pi / 180.0) - \
              (x0 - rotation_center_x) * numpy.sin(delta_ang * numpy.pi / 180.0)
        rxB = -1 * (x0 - rotation_center_x) * numpy.cos(delta_ang * numpy.pi / 180.0) + \
              (y0 - rotation_center_y) * numpy.sin(delta_ang * numpy.pi / 180.0)
        ryB = (y0 - rotation_center_y) * numpy.cos(delta_ang * numpy.pi / 180.0) - \
              (-1) * (x0 - rotation_center_x) * numpy.sin(delta_ang * numpy.pi / 180.0)
...
def generate_color_filter4(size):
    ...
    for y in range(height):
        for x in range(width):
            #            my_filter[y][x] = numpy.sqrt(numpy.sqrt((x-center_x)**2+(y-center_y)**4))/ factor
            my_filter[y][x] = numpy.sqrt(numpy.sqrt(x ** 4 + y ** 4)) / factor
    return my_filter
...
def generate_color_filter2(size):
    ...
    for y in range(height):
        for x in range(width):
            #            my_filter[y][x] = numpy.sqrt((x-center_x)**2+(y-center_y)**2)/ factor
            my_filter[y][x] = numpy.sqrt(x ** 2 + y ** 2) / factor
    return my_filter


# noise should be a bidimensional array
def generate_colored_noise(my_filter):
    shape = my_filter.shape
    #    pure_noise = numpy.random.normal(size=shape)
    pure_noise = numpy.random.uniform(low=-1.0, high=1.0, size=shape)
    filtered_noise = pure_noise * my_filter
...
def load_raw_data(data_base_dir="/scratch/escalafl/cooperations/igel/patches_8x8",
                  base_filename="bochum_natural_8_5000.bin", input_dim=64, dtype="uint8", select_samples=None,
                  verbose=False):
    filename = data_base_dir + "/" + base_filename
    fd = open(filename, 'rb')
    #    s_header = fd.read(16)
...
# my_filter = numpy.ones((16,16))
# for x in range(16):
#    for y in range(16):
#        my_filter[y][x] = numpy.sqrt((x-8)**2+(y-8)**2)/ (8 * 1.72)
#
# my_filter2 = numpy.ones((16,16))
# for x in range(16):
#    for y in range(16):
#        my_filter2[y][x] = ((x-8)**2+(y-8)**2)/ ((8 * 1.72)**2)
#
# noise = numpy.random.normal(size=(16,16))
# col_noise = noise * my_filter
# col_noise2 = noise * my_filter2
#
