...
class ParamsNetwork(object):
    ...
    def __values__(self):
        """This function returns the values used for caching (to compute the hash of the object). """
...
class ParamsSFALayer(ParamsSFASuperNode):
    ...
    A layer is composed of the following elements (all optional except the Switchboard):
    PInvSwitchboard, pca_node, ord_node, gen_exp, red_node, clip_node, sfa_node.
    """
    def __init__(self):
...
class ExperimentResult(object):
    """Stores relevant variables and results generated by an excecution of Cuicuilco.
...
def test_object_contents(an_object, verbose=False):
    ...
    if len(list_none_elements) > 0 and verbose:
        print("Warning!!! object %s contains 'None' fields: " % (str(an_object)), list_none_elements)


def scale_sSeq(sSeq, reduction_factor, verbose=False):
    """Scales a concrete data description (reduces the resolution).
...
def convert_sSeq_to_funcs_params_sets(sSeq, verbose=True):
    ...
    """
    print("conversion of sSeq to funcs and params sets:", sSeq)
    if isinstance(sSeq, list):
        funcs_sets = []
        params_sets = []
        for sSeq_vect in sSeq:
...
def load_data_from_sSeq(self):
    ...
    Particular experiments may implement their own 'load_data' function instead of using this function.
    """
    seq = self
    if seq.input_files == "LoadBinaryData00":
        ...
        data = image_loader.load_raw_data(seq.data_base_dir, seq.base_filename, input_dim=seq.input_dim,
                                          dtype=seq.dtype, select_samples=seq.samples, verbose=False)
    else:
        data = image_loader.load_image_data(seq.input_files, seq.images_array, seq.image_width, seq.image_height,
                                            seq.subimage_width, seq.subimage_height, seq.pre_mirror_flags,
                                            seq.pixelsampling_x, seq.pixelsampling_y,
                                            ...
                                            seq.trans_sampled, seq.rotations, seq.rotate_before_translation,
                                            seq.contrast_enhance, seq.obj_avgs,
    ...
    if isinstance(train_data_sets, list):
        if isinstance(train_data_sets[0], list):
            if len(train_data_sets[0]) == 1:
                ...
                er = "It is not possible to add additional labels for this training data"
                raise Exception(er)
    else:
