...
#####################################################################################################################
def add_edge(edge_weights, n1, n2, w): #Adds an edge to an edge_weight dictionary
    if (n1,n2) in edge_weights.keys():
        edge_weights[(n1,n2)] += w
    else:
        edge_weights[(n1,n2)] = w

def GenerateLinearGraphConsistent(num_samples): #Computes a linear graph. Self-loops added to first and last samples for consistency constraints
    N = num_samples
...
def GenerateClusteredGraph(num_samples_per_cluster): #Generates a clustered graph, consistent, without self-loops
...
def GenerateCompactClassesGraph(label_block_size, J): #Graph for dense classification features, C = 2^J only, binary labels 0,1, no self-loops
    num_classes = 2 ** J
...
def GenerateSlidingWindowConsistentGraph(num_samples, d): #Self-loops added in the end to fix consistency
    N = num_samples
...
def GenerateImprovedGraph(num_samples, block_size, balance_factor=1.0): #One attempt at semi-supervised learning
    N = num_samples
    num_blocks = N/block_size
...
def scale_edge_weights(edge_weights, w): #Scale whole edge-weights dictionary
    edge_weights2 = {}
    for (n1,n2) in edge_weights.keys():
        edge_weights2[(n1,n2)] = w * edge_weights[(n1,n2)]
    return edge_weights2

def add_unsupervised_samples(node_weights1, edge_weights1, N2, w1=1.0, w2=1.0): #Another attempt at semi-supervised learning
...
#####################################################################################################################
def IsGraphConsistent(node_weights, edge_weights): #Verifies consistency constraint on given graph
...
def ExtractGammaFromGraph(node_weights, edge_weights): #Creates matrix Gamma from edge-weight dictionary
    N = len(node_weights)
...
def ExtractEigenvectors(M): #Computes eigenvectors, ordered by eigenvalues. Sing fixed to that eigenvectors start <= 0
...
def ComputeFreeResponses(eigv, node_weights): #Maps eigenvectors to free responses 
    Q = ComputeQ(node_weights)
    node_weights_m12 = numpy.diag(node_weights ** -0.5)
    return (Q**0.5)*numpy.dot(node_weights_m12, eigv)

def Extract_Deltas_from_Eigenvalues(l, Q, R): #Maps eigenvalues to delta values
    return 2-2.0/R*l*Q #error here!!!????? mixing of two features, both with D=0

#TODO: Check this function, there might be an easier way
def EnforceEigenvectorConst(l,eigv, node_weights): #makes sure the last eigenvector corresponds to constant free response, and decorrelates it from everything else
    Q = ComputeQ(node_weights)
...
#        eigv[:, j] = eigv[:,j]/((eigv[:,j]**2).sum()**0.5)
#    print "Re-normalized eigenvectors: eigvT=", eigv    
...
def MapLabelsToFreeResponses(labels, node_weights):
    ...
    #Decorrelate Labels, and normalize amplitude
    for j in range(J+1):
...
#            free_responses[:,k] = free_responses[:,k] - numpy.dot(free_responses[:,k], free_responses[:,j]) * free_responses[:,j] / norm_free_response**2        
