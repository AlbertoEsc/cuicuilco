...
num_subimages = len(posX_values) * len(posY_values)
subimage_coordinates = numpy.zeros((num_subimages, 4))

#subimage_width, subimage_height
for j, posY in enumerate(posY_values):
    for i, posX in enumerate(posX_values):
        subimage_coordinates[j*len(posX_values)+i] = numpy.array([posX, posY, posX+patch_width-1, posY+patch_height-1])

#print "subimage_coordinates", subimage_coordinates
base_estimation = subimage_coordinates + 0.0
base_magnitude = patch_width **2 + patch_height**2
images_ext = []
for image in images:
    for xx in range(num_subimages):
        images_ext.append(image)

#subimage_coordinates = subimage_coordinates times num_images
#image = images[0] #TODO => loop

invalid_subimages = numpy.zeros(num_subimages, dtype='bool')
colors = numpy.random.uniform(0.0, 1.0, size=(num_subimages,3))
...
im_disp = numpy.asarray(images_ext[sel_image])
...
for ii, (x0, y0, x1, y1) in enumerate(subimage_coordinates):
#    p11.plot([x0, x1, x1, x0, x0], [y0, y0, y1, y1, y0], color=colors[ii] )
    p11.plot([x0, x1], [y0, y1], color=colors[ii] )
...
for i in range(num_networks):
    #Get arrays
    subimages = extract_subimages(images_ext, subimage_coordinates)
    subimages_arr = images_asarray(subimages)+0.0
    t_afterloading = time.time()

    sl = networks[i].execute(subimages_arr)
    ...
    if i%4 == 0: #POS_X       
        width = subimage_coordinates[:, 2] - subimage_coordinates[:, 0]
        reg_out = reg_out * width / regression_width
#        print "Regression Output scaled:", reg_out
#        print "Correcting coordinates (X)"
        subimage_coordinates[:, 0] = subimage_coordinates[:, 0] - reg_out  #X0
        subimage_coordinates[:, 2] = subimage_coordinates[:, 2] - reg_out  #X1
    elif i%4 == 1:     #POS_Y
        height = subimage_coordinates[:, 3] - subimage_coordinates[:, 1]
        reg_out = reg_out * height / regression_height
#        print "Regression Output scaled:", reg_out
#        print "Correcting coordinates (Y)"
        subimage_coordinates[:, 1] = subimage_coordinates[:, 1] - reg_out  #Y0
        subimage_coordinates[:, 3] = subimage_coordinates[:, 3] - reg_out  #Y1
    elif i%4 == 2: #SCALE
        old_width = subimage_coordinates[:, 2] - subimage_coordinates[:, 0]
        old_height = subimage_coordinates[:, 3] - subimage_coordinates[:, 1]
        x_center = (subimage_coordinates[:, 2] + subimage_coordinates[:, 0])/2.0
        y_center = (subimage_coordinates[:, 3] + subimage_coordinates[:, 1])/2.0
...
#        print "Correcting scale (X)"
        subimage_coordinates[:, 0] = x_center - width / 2.0
        subimage_coordinates[:, 2] = x_center + width / 2.0
        subimage_coordinates[:, 1] = y_center - height / 2.0
        subimage_coordinates[:, 3] = y_center + height / 2.0
    else:
        pass #regression does not need to modify subimage coordinates

    if i%4 in [0,1,2]: 
        new_wrong_images = (subimage_coordinates[:,0]<0) | (subimage_coordinates[:,1]<0) | \
        (subimage_coordinates[:,2]>=im_width) | (subimage_coordinates[:,3]>=im_height) | \
        (((subimage_coordinates - base_estimation)**2).sum(axis=1) > 2*base_magnitude)
    else: #Classifier
        cut_off_face = 0.0
        new_wrong_images = reg_out >= cut_off_face
    
    subimage_coordinates[new_wrong_images] = numpy.array([0, 0, 1 ,1])
    invalid_subimages = invalid_subimages | new_wrong_images

    subplot = subplots[i]
    subplot.imshow(im_disp, aspect='auto', interpolation='nearest', origin='upper', cmap=mpl.pyplot.cm.gray)
    for j, (x0, y0, x1, y1) in enumerate(subimage_coordinates):
        if invalid_subimages[j] == False and False:
            subplot.plot([x0, x1, x1, x0, x0], [y0, y0, y1, y1, y0], color=colors[j] )
        if invalid_subimages[j] == False:
            subplot.plot([x0, x1], [y0, y1], color=colors[j] )
...
print "%d / %d valid images"%(len(invalid_subimages)-invalid_subimages.sum(), len(invalid_subimages))
plt.show()
